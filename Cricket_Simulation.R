...
cricket_sim <- function(country, input_file, output_file, n_runs, strata_sizes){
  # Function to prep cricket data and run simulation:
  #   country is which country to run
  ...
  # Drop row of selected country
  i1 <- i1[-which(str_detect(i1[,1], country)),]
  i2 <- i2[-which(str_detect(i2[,1], country)),]
...
# Get command line arguments (base_path, selected_country, n_iter)
base_path <- args[1] 
...
variables <- tibble(
  ...
  strata_count = list(
    c(1,2,4,2,1,1), c(2,1,4,2,1,1), c(2,3,3,1,2), c(3,1,3,2,1,1),
    c(1,1,4,2,1,2), c(2,3,1,4,1), c(2,1,3,3,1,1), c(1,5,3,1,1),
    c(2,3,2,2,1,1), c(2,2,3,1,1,2), c(2,2,3,2,1,1), c(1,5,2,2,1)
  )
) %>% 
  mutate(
...
variables <- filter(variables, country == selected_country)
