...
inddat<-read.csv("/Users/quazi/Desktop/cric/IPL/csk.csv",header=T)
head(inddat)
type<-inddat$roles
type
...
autocsk<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(1,1,2,3,3,1),method = "srswor")
...
csksims<-replicate(5,autocsk())

cs<-data.frame(matrix(unlist(csksims), nrow=length(csksims), byrow=T))
cs
colnames(cs)<-c("v Delhi Capitals", "v Kings XI Punjab" , "v Kolkata Knight Riders",
                "v Mumbai Indians", "v Rajasthan Royals", "v Royal Challengers Bangalore",
                "v Sunrisers Hyderabad")
setwd("/Users/quazi/Desktop/cric/IPL")
write.csv(cs,'cs.csv')
cs
mean(as.numeric(re[,1]<cs[,6]))
re[,1]
