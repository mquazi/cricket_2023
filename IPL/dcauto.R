...
inddat<-read.csv("/Users/quazi/Desktop/cric/IPL/dc.csv",header=T)
head(inddat)
type<-inddat$roles
type
...
autodc<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(1,2,1,3,3,1),method = "srswor")
...
dcsims<-replicate(5,autodc())

ds<-data.frame(matrix(unlist(dcsims), nrow=length(dcsims), byrow=T))
ds
colnames(ds)<-c("v Chennai Super Kings", "v Kings XI Punjab" , 
                "v Kolkata Knight Riders", "v Mumbai Indians",  
                "v Rajasthan Royals", "v Royal Challengers Bangalore",
