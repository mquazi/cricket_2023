...
inddat<-read.csv("/Users/quazi/Desktop/cric/IPL/kkr.csv",header=T)
head(inddat)
type<-inddat$roles
type
...
autokkr<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(1,1,2,2,3,2),method = "srswor")
...
kkrsims<-replicate(5,autokkr())

ks<-data.frame(matrix(unlist(kkrsims), nrow=length(kkrsims), byrow=T))
ks
colnames(ks)<-c("v Chennai Super Kings", "v Delhi Capitals", "v Kings XI Punjab" ,
