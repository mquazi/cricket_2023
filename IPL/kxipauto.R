...
inddat<-read.csv("/Users/quazi/Desktop/cric/IPL/kxip.csv",header=T)
head(inddat)
type<-inddat$roles
type
...
autokxip<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(1,1,1,4,3,1),method = "srswor")
...
kxipsims<-replicate(5,autokxip())

kb<-data.frame(matrix(unlist(kxipsims), nrow=length(kxipsims), byrow=T))
kb
colnames(kb)<-c("v Chennai Super Kings", "v Delhi Capitals", "v Kolkata Knight Riders", 
                "v Mumbai Indians", "v Rajasthan Royals", "v Royal Challengers Bangalore",
