...
inddat<-read.csv("/Users/quazi/Desktop/cric/IPL/mi.csv",header=T)
head(inddat)
type<-inddat$roles
type
...
automi<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(1,2,3,3,1,1),method = "srswor")
...
misims<-replicate(5,automi())

ms<-data.frame(matrix(unlist(misims), nrow=length(misims), byrow=T))
ms
colnames(ms)<-c("v Chennai Super Kings", "v Delhi Capitals", "v Kings XI Punjab" , 
