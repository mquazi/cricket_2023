...
inddat<-read.csv("/Users/quazi/Desktop/cric/IPL/rcb.csv",header=T)
head(inddat)
type<-inddat$roles
type
...
autorcb<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(1,1,1,2,3,3),method = "srswor")
...
rcbsims<-replicate(5,autorcb())

re<-data.frame(matrix(unlist(rcbsims), nrow=length(rcbsims), byrow=T))
re
colnames(re)<-c("v Chennai Super Kings", "v Delhi Capitals", "v Kings XI Punjab" , "v Kolkata Knight Riders",
                 "v Mumbai Indians", "v Rajasthan Royals", "v Sunrisers Hyderabad")
