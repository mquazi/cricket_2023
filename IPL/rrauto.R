...
inddat<-read.csv("/Users/quazi/Desktop/cric/IPL/rr.csv",header=T)
head(inddat)
type<-inddat$roles
type
...
autorr<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(2,2,2,1,3,1),method = "srswor")
...
rrsims<-replicate(5,autorr())

rs<-data.frame(matrix(unlist(rrsims), nrow=length(rrsims), byrow=T))
rs
colnames(rs)<-c("v Chennai Super Kings", "v Delhi Capitals","v Kings XI Punjab" , 
