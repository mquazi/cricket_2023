...
inddat<-read.csv("/users/dattap/cricket_2020/afghanistan/afg.csv",header=T)
head(inddat)
str(inddat)
...
autoafg<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(1,2,4,2,1,1), method = "srswor")
...
an
colnames(an)<-c("v Australia", "v Bangladesh","v Bermuda", "v Canada",
                "v England",
...
setwd("/users/dattap/cricket_2020/afghanistan/")
write.csv(an,'an.csv')
