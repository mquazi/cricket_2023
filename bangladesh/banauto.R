...
inddat<-read.csv("/users/dattap/cricket_2020/bangladesh/bangla.csv",header=T)
head(inddat)
str(inddat)
...
autoban<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(2,3,3,1,2),method = "srswor")
...
bh
colnames(bh)<-c("v Afghanistan","v Australia", "v Bermuda", "v Canada","v England",
