...
inddat<-read.csv("/users/dattap/cricket_2020/england/eng.csv",header=T)
head(inddat)
str(inddat)
...
autoeng<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(3,1,3,2,1,1),method = "srswor")
...
ed
colnames(ed)<-c("v Afghanistan","v Australia","v Bangladesh",   "v Bermuda", "v Canada",
...
setwd("/users/dattap/cricket_2020/england/")
write.csv(ed,'ed.csv')
