...
inddat<-read.csv("/users/dattap/cricket_2020/newzealand/nzl.csv",header=T)
head(inddat)
str(inddat)
...
autonzl<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(2,1,3,3,1,1),method = "srswor")
...
nd
colnames(nd)<-c("v Afghanistan","v Australia", "v Bangladesh","v Bermuda", "v Canada",
                "v England",
...
setwd("/users/dattap/cricket_2020/newzealand/")
write.csv(nd,'nd.csv')
