...
inddat<-read.csv("/users/dattap/cricket_2020/southafrica/saf.csv",header=T)
head(inddat)
str(inddat)
...
autosaf<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(2,3,2,2,1,1), method = "srswor")
...
sa
colnames(sa)<-c("v Afghanistan","v Australia", "v Bangladesh","v Bermuda", "v Canada",
                "v England",
...
setwd("/users/dattap/cricket_2020/southafrica/")
write.csv(sa,'sa.csv')
