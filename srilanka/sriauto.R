...
inddat<-read.csv("/users/dattap/cricket_2020/srilanka/sri.csv",header=T)
head(inddat)
str(inddat)
...
autosri<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(2,2,3,1,1,2), method = "srswor")
...
ska
colnames(ska)<-c("v Afghanistan","v Australia", "v Bangladesh","v Bermuda", "v Canada",
                "v England",
                "v Hong Kong","v India","v Ireland","v Kenya","v Netherlands",
                "v New Zealand", "v Pakistan","v Scotland", "v South Africa", "v U.A.E.", 
