...
inddat<-read.csv("/users/dattap/cricket_2020/zimbabwe/zim.csv",header=T)
head(inddat)
str(inddat)
...
autozim<-function(){
        select<-strata(g2,stratanames=c("myfactor"),size=c(1,5,2,2,1), method = "srswor")
...
ze
colnames(ze)<-c("v Afghanistan","v Australia", "v Bangladesh","v Bermuda", "v Canada",
                "v England",
                "v Hong Kong","v India","v Ireland","v Kenya","v Netherlands",
                "v New Zealand", "v Pakistan","v Scotland", "v South Africa", 
                "v Sri Lanka" ,"v U.A.E.","v West Indies")
setwd("/users/dattap/cricket_2020/zimbabwe/")
write.csv(ze,'ze.csv')
